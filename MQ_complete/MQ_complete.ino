...
#define         MQ2_SENSOR                   (A0)  //define which analog input channel you are going to use
//#define         MQ6_SENSOR                   (1)
...
void setup()
{
  ...
  Ro0 = MQCalibration(MQ2_SENSOR, 0.5, RL0,C2H5OH_terCurve);
  Ro0 = MQCalibration(MQ2_SENSOR, 0.5, RL0,CH4_secCurve);
}
void loop()
{
  //MQ8 CO LPG Smoke;
  ...
  Serial.println(MQGetGasPercentage(MQRead(MQ2_SENSOR, RL0), Ro0, GAS_C2H5OH, MQ2_SENSOR) );
...
float MQCalibration(int mq_pin, double ppm, double rl_value, float *pcurve )
{
  ...
  val = val / CALIBRATION_SAMPLE_TIMES;MQR                 //calculate the average value
